  ...
  return (
    <div className="relative">
      <button onClick={() => setShowMenu(!showMenu)} className="btn-outline w-full flex items-center justify-center gap-2 text-xs md:text-sm">
        {copied ? <Check size={14} className="text-green-600" /> : <Share2 size={14} />}{copied ? 'Tersalin!' : 'Bagikan Hasil'}
      </button>
      {showMenu && (<>
        ...
        <div className="absolute bottom-full left-0 right-0 mb-2 z-50 bg-white dark:bg-zinc-900 border border-wr-border dark:border-zinc-700 shadow-lg">
          <div className="p-3 border-b border-wr-border dark:border-zinc-800"><p className="text-[10px] font-mono text-wr-gray dark:text-zinc-500 uppercase tracking-widest mb-2">Preview</p><div className="text-xs text-wr-black dark:text-zinc-300 bg-wr-surface dark:bg-zinc-800 p-3 font-mono whitespace-pre-line leading-relaxed max-h-32 overflow-y-auto">{teks}</div></div>
    ...
    <div className="px-4 sm:px-6 py-12 md:py-16">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-2 md:gap-3 mb-4">
          {[{ val: skor, label: 'Benar', color: 'text-green-600 dark:text-green-400' },{ val: total - skor, label: 'Salah', color: 'text-red-600 dark:text-red-400' },{ val: `${persen}%`, label: 'Skor', color: 'text-wr-black dark:text-white' }].map((s) => (
        ...
        {hasilPoin && !modeLatihan && (
          <div className="border border-wr-border dark:border-zinc-800 p-4 md:p-5 mb-4 bg-white dark:bg-zinc-900">
            ...
            {hasilPoin.streakMultiplier > 1 && <p className="text-[10px] md:text-xs font-mono text-amber-600 dark:text-amber-400 mb-2 flex items-center gap-1"><Flame size={12} />Streak {hasilPoin.streak} hari — bonus ×{hasilPoin.streakMultiplier}</p>}
            <div className="flex items-center justify-between text-xs font-mono text-wr-gray dark:text-zinc-500"><span>{hasilPoin.kelasSesudah.nama}</span><span>{hasilPoin.totalPoin} poin total</span></div>
            {hasilPoin.naikKelas && (<div className="mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-center"><p className="text-sm font-bold text-amber-800 dark:text-amber-300">🎉 Naik Kelas!</p><p className="text-xs font-mono text-amber-700 dark:text-amber-400 mt-1">{hasilPoin.kelasSebelum.nama} → {hasilPoin.kelasSesudah.nama}</p></div>)}
          </div>
        )}
        <div className="mb-4"><TombolBagikan skor={skor} total={total} persen={persen} namaKategori={namaKategori} level={level.label} /></div>
        <button onClick={() => setLihatReview(!lihatReview)} className="w-full btn-outline mb-6 text-xs md:text-sm">{lihatReview ? 'Sembunyikan Review' : 'Lihat Review Jawaban'}</button>
        {lihatReview && (
                ...
                <div key={item.id} className={`border p-4 md:p-5 ${benar ? 'border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20' : 'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20'}`}>
                  <div className="flex items-start gap-2 md:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`text-[10px] md:text-xs px-2 md:px-3 py-1.5 md:py-2 mb-2 font-mono break-words ${benar ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}><span className="font-bold uppercase tracking-widest">{benar ? 'Benar:' : 'Salah:'}</span> {item.dipilih !== null ? item.pilihan[item.dipilih] : 'Tidak dijawab'}</div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white dark:bg-zinc-900 border border-wr-border dark:border-zinc-700 p-6 md:p-8 max-w-sm w-full mx-4">
        <div className="flex items-center gap-3 mb-3 md:mb-4"><AlertTriangle size={18} className="text-wr-red flex-shrink-0" /><h3 className="text-sm md:text-base font-bold text-wr-black dark:text-white">Batalkan Tes?</h3></div>
...
export default function EvaluasiTes() {
  ...
  const [showCountdown, setShowCountdown] = useState(!modeLatihan)
  ...
  useEffect(() => {
    ...
    setShowCountdown(!modeLatihan)
    setTimerAktif(false)
  }, [data, kategori, modeLatihan])

  // Timer — hanya mulai setelah countdown selesai
  useEffect(() => {
    if (!timerAktif || selesai || soalAcak.length === 0 || modeLatihan) return
    const iv = setInterval(() => {
      setWaktu((p) => { if (p <= 1) { clearInterval(iv); setSelesai(true); return 0 } return p - 1 })
    }, 1000)
    return () => clearInterval(iv)
  }, [timerAktif, selesai, soalAcak, modeLatihan])
  ...
  const handleCountdownSelesai = useCallback(() => {
  ...
  const handleUlang = useCallback(() => {
    setSoalAcak(acakSoalDanPilihan(data.soal))
    setSoalIndex(0); setPilihanDipilih(null); setRiwayatJawaban([]); setSelesai(false); setShowFeedback(false); setHasilPoin(null)
    setWaktu(modeLatihan ? 0 : data.durasi)
    sudahSimpanRef.current = false
    if (!modeLatihan) { setShowCountdown(true); setTimerAktif(false) }
  }, [data, modeLatihan])

  // Countdown
  if (showCountdown && data) {
    return <Countdown onSelesai={handleCountdownSelesai} namaTest={data.nama} />
  }
  ...
  const handleSelanjutnya = () => {
    ...
    setRiwayatJawaban([...riwayatJawaban, { ...soalSaat, dipilih: pilihanDipilih }])
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-zinc-950">
      ...
      <div className="sticky top-16 z-40 bg-white dark:bg-zinc-950 border-b border-wr-border dark:border-zinc-800">
        {hampirHabis && (<div className={`flex items-center justify-center gap-2 py-1.5 md:py-2 text-[10px] md:text-xs font-mono font-bold tracking-widest uppercase ${sangatMendekat ? 'bg-red-600 text-white' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 border-b border-amber-200 dark:border-amber-800'}`}><Clock size={10} /><span className="truncate">{sangatMendekat ? `WAKTU HAMPIR HABIS — ${timerStr}` : `Sisa < 1 menit — ${timerStr}`}</span></div>)}
        <div className="page-container"><div className="flex items-center justify-between h-10 md:h-12">
          <div className="flex items-center gap-2 truncate mr-4"><span className="text-xs md:text-sm font-bold text-wr-black dark:text-white truncate">{data.nama}</span><span className={`text-[10px] font-mono font-bold tracking-widest uppercase px-1.5 py-0.5 border flex-shrink-0 ${modeLatihan ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800' : 'bg-wr-muted text-wr-gray border-wr-border dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700'}`}>{modeLatihan ? 'Latihan' : 'Evaluasi'}</span></div>
